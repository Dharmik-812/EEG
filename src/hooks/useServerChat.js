...
export default function useServerChat() {
    ...
    // API endpoint - use relative path for production, full URL for development if needed
    ...
    const sendMessage = useCallback(async (rawInput) => {
        ...
        try {
            ...
            try {
                ...
                // Call server API with retry logic
                const result = await executeWithRetry(async () => {
                    return await callServerAPI(contents, modelKey, false)
                })
    ...
    const sendMessageWithImage = useCallback(async (rawInput, imageData) => {
        ...
        try {
            ...
            try {
                ...
                const result = await executeWithRetry(async () => {
                    return await callServerAPI(contents, modelKey, true)
                })
    ...
    // API status - simplified since we're using server endpoint
    const apiStatus = 'ready' // Server handles API key validation
