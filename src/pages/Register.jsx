...
export default function Register() {
  ...
  // step is implicit via audience/role; preserved for future flow
  // eslint-disable-next-line no-unused-vars
  const [step, setStep] = useState(1)
  const [audience, setAudience] = useState('visitor') // visitor | school | college
  const [role, setRole] = useState('visitor') // visitor | school-student | school-teacher | college-student | college-teacher
  const [institution, setInstitution] = useState({ name: '', type: '' })
  const navigate = useNavigate()

  function onSubmit(e) {
    e.preventDefault()
    try {
      const selectedRole = audience === 'visitor' ? 'visitor' : role
      const inst = audience === 'visitor' ? undefined : { name: institution.name, type: audience }
      register(name, email, password, selectedRole, inst)
          ...
          <Card>
            ...
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-3">
                <div className="text-sm font-semibold text-slate-700 dark:text-slate-200">I am joining as</div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {[
                    ...
                    <button
                      type="button"
                      key={o.id}
                      onClick={() => { setAudience(o.id); setStep(o.id === 'visitor' ? 3 : 2) }}
                      className={`p-3 rounded-lg border transition-colors ${audience === o.id ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20' : 'border-slate-300 dark:border-slate-700'}`}
                    >{o.label}</button>
                  ))}
                </div>
              </motion.div>

              {/* Step 2: Role & institution (for school/college) */}
              {audience !== 'visitor' && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-3">
                  <div className="text-sm font-semibold text-slate-700 dark:text-slate-200">Select your role</div>
                  <div className="grid grid-cols-2 gap-3">
                    {(audience === 'school' ? [
                      { id: 'school-student', label: '🧒 Student' },
                      { id: 'school-teacher', label: '👨‍🏫 Teacher' },
                    ] : [
                      { id: 'college-student', label: '🎓 Student' },
                      { id: 'college-teacher', label: '👩‍🏫 Teacher' },
                    ]).map((o) => (
                      <button type="button" key={o.id} onClick={() => { setRole(o.id); setStep(3) }} className={`p-3 rounded-lg border transition-colors ${role === o.id ? 'border-sky-500 bg-sky-50 dark:bg-sky-900/20' : 'border-slate-300 dark:border-slate-700'}`}>{o.label}</button>
                    ))}
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input className="rounded-lg border border-slate-300 dark:border-slate-700 px-4 py-3 bg-white/80 dark:bg-slate-800/80" placeholder={`${audience === 'school' ? 'School' : 'College'} name`} value={institution.name} onChange={e => setInstitution({ ...institution, name: e.target.value, type: audience })} required />
                  </div>
                </motion.div>
              )}
              <motion.div
